...
class Parser:
    ...
    def __init__(self, grammar="pcfg.txt"):
        """
    ...
    def nltk_test(self, tokenized_sentences="nltk_test.txt"):
        with open(tokenized_sentences, "r") as f:
            nltk_test_sentences = [line for line in f]

        for test_sentence in tqdm(nltk_test_sentences, desc="NLTK Test sentence"):
            tokens = word_tokenize(test_sentence)
            print(tokens)
            CKY.parse(tokens)
            CKY.print_tree(write=True, draw=False)
    ...
    def parse(self, tokens):
        ...
        for j, word in enumerate(tqdm(tokens, desc="Tokens")):
    ...
    def print_tree_written(self, final_nodes):
        ...
        for i in range(len(write_trees)):
            print(write_trees[i])
            print("the possibility of this parse is ", poss_trees[i])
        print(
...
if __name__ == "__main__":
    ...
    if DEMO:
        CKY = Parser("demo_grammar.txt")
        # sentence = "I shot an elephant in my pajamas"
        sentence = "Book the flight through Houston"
        tokens = word_tokenize(sentence)
        print(tokens)
        CKY.parse(tokens)
        CKY.print_tree(write=True, draw=True)

    else:
        data = Dataset()
        CKY = Parser()
        # CKY.nltk_test()
        for i, sentence in enumerate(tqdm(data.test_sentences, desc="Test sentence")):
            tokens = word_tokenize(sentence)
            print(tokens)
            CKY.parse(tokens)
            CKY.print_tree(write=True, draw=False)
